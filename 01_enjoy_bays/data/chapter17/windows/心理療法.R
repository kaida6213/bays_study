#Stan2.17,rstan2.17.3, Rtools34, blavaan 0.3-1, R3.5.1, windows 8.1.1で動作確認済,2018,08,22
...
fit1.growth.ml<-lavaan::growth(LGM,data=dat) # 最尤法と解こうとすると因子の分散が負値
...
qIm<-ggplot(alpha_I,aes(x=x))+geom_histogram(alpha=.6,col="white",bins=40,aes(y=..density..), position="identity") + 
  geom_density(aes(x=x,y=..density..),col="black",alpha=.7)+xlab("切片因子の平均")+theme_bw(base_family = "MEI")
qSm<-ggplot(alpha_I,aes(x=x))+geom_histogram(alpha=.6,col="white",bins=40,aes(y=..density..), position="identity") + 
  geom_density(aes(x=x,y=..density..),col="black",alpha=.7)+xlab("傾き因子の平均")+theme_bw(base_family = "MEI")

qIv<-ggplot(psi_I,aes(x=x))+geom_histogram(alpha=.6,col="white",bins=40,aes(y=..density..), position="identity") + 
  geom_density(aes(x=x,y=..density..),col="black",alpha=.7)+xlab("切片因子の分散")+theme_bw(base_family = "MEI")
qSv<-ggplot(psi_I,aes(x=x))+geom_histogram(alpha=.6,col="white",bins=40,aes(y=..density..), position="identity") + 
  geom_density(aes(x=x,y=..density..),col="black",alpha=.7)+xlab("傾き因子の分散")+theme_bw(base_family = "MEI")


qIbeta<-ggplot(beta_I,aes(x=x))+geom_histogram(alpha=.6,col="white",bins=40,aes(y=..density..), position="identity") + 
  geom_density(aes(x=x,y=..density..),col="black",alpha=.7)+xlab("共変量から切片因子への回帰係数")+theme_bw(base_family = "MEI")
qSbeta<-ggplot(beta_S,aes(x=x))+geom_histogram(alpha=.6,col="white",bins=40,aes(y=..density..), position="identity") + 
  geom_density(aes(x=x,y=..density..),col="black",alpha=.7)+xlab("共変量から傾き因子への回帰係数")+theme_bw(base_family = "MEI")
...
qplot(x=dat$BaseSuppression,y=predict(fit1.growth.stan)[,2])+theme_classic(base_family = "MEI")+ylim(2,-9)+
